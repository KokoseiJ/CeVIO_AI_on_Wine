...
int init_wbemsvc() {
	...
	if (FAILED(hres)) {
		debug_info(funcname, "Failed to initialize COM: 0x%08lx\n", hres);
		return 1;
	}
	...
	if (FAILED(hres)) {
		debug_info(funcname, "Failed to initialize Security: 0x%08lx\n", hres);
		CoUninitialize();
		return 1;
	}
	*/
	...
	if (FAILED(hres)) {
		debug_info(funcname, "Failed to create IWbemLocator: 0x%08lx\n", hres);
		CoUninitialize();
		return 1;
	}
	...
	if (FAILED(hres)) {
		debug_info(funcname, "Failed to connect to CIMV2 WMI namespace: 0x%08lx\n", hres);
		pLoc->Release();
		CoUninitialize();
		return 1;
	}
	...
	if (FAILED(hres)) {
		debug_info(funcname, "Failed to set Proxy Blanket: 0x%08lx\n", hres);
		pSvc->Release();
		pLoc->Release();     
		CoUninitialize();
		return 1;
